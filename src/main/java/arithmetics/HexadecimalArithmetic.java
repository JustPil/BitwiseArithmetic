...
public class HexadecimalArithmetic {
    private final int HEXADECIMAL_BASE = 16;
    private final Map<Character, Integer> mapGetInt = new HashMap<>();
    private final Map<Integer, Character> mapGetChar = new HashMap<>();

    /**
     * Constructor calls the populateMap method to populate the class's Map variables.
     */
    public HexadecimalArithmetic() {
        populateIntMap();
        populateCharMap();
    }
     ...
     * @return The decimal conversion of the hexadecimal number, or -1 if the input was invalid.
     */
    public int hexToDecimal(String s) {
        int result = 0, counter = 0;
        for(int i = s.length() - 1; i >= 0; i--) {
            if(!mapGetInt.containsKey(s.charAt(i))) {
                return -1;
            }
    ...
    public String addHexStrings(String s1, String s2) {
        ...
        for(int i = s1.length() - 1; i >= 0; i--) {
            if(!mapGetInt.containsKey(s1.charAt(i)) || !mapGetInt.containsKey(s2.charAt(i))) {
                return "invalid";
            }
    ...
    public String multiplyHex(char c, String s) {
        ...
        for(int i = s.length() - 1; i >= 0; i--) {
            if(!mapGetInt.containsKey(s.charAt(i)) || !mapGetInt.containsKey(c)) {
                return "invalid";
            }
    ...
    /**
     * populateIntMap Populates the class's Map variable with hexadecimal characters and their decimal equivalents.
     */
    private void populateIntMap() {
        mapGetInt.put('0', 0);
        mapGetInt.put('1', 1);
        mapGetInt.put('2', 2);
        mapGetInt.put('3', 3);
    ...
    /**
     * populateCharMap Populates the class's Map variable with hexadecimal characters and their decimal equivalents.
     */
